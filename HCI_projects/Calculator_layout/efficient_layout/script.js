document.addEventListener('DOMContentLoaded', () => {
    // gets all <p> elements in class buttons
    ...
    let firstNumberOnScreen = false, secondNumberOnScreen = false;
    let equalOnScreen = false;
    let number1, number2, result, sum;
    let subterm;
    let mult1 = false;
    let starter = true;
    ...
    function saveResultsToFile() {
        let textContent = results.join('\n');
        const blob = new Blob([textContent], { type: 'text/plain' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'efficient_layout.txt';
